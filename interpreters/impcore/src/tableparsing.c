...
/* private function prototypes for parsing S211f */
static bool rowmatches(struct ParserRow *row, Name first);
/* private function prototypes for parsing S216c */
...
ParserResult sNamelist(ParserState s) {
    if (s->input == NULL) {
        ...
        case LIST:
            halfshift(s);
            s->components[s->nparsed++].names = parsenamelist(p->list, &s->
                                                                       context);
            return PARSED;
        }
        assert(0);
