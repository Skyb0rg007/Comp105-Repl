...
fun atLoc loc f a =
  f a handle e as RuntimeError _ => raise Located (loc, e)
           ...
           | e as IO_Io _   => raise Located (loc, e)
...
(*#line 11 "tuschemeans.nw"*)fun typeof (e, Delta, Gamma) =
  let fun literal NIL           = FORALL (["'a"], listtype tyvarA)
        | literal (PAIR (h, t)) = listLiteral (literal h) t
        | literal (BOOLV _) = booltype
        | literal (NUM  _) = inttype
        | literal (SYM  _) = symtype
        | literal _ = let exception ThisCan'tHappen in raise ThisCan'tHappen end
                         (* code above covers all real cases *)
      and listLiteral tau NIL = listtype tau
        | listLiteral tau (PAIR (h, t)) =
            if eqType(tau, literal h) then listLiteral tau t
            else raise TypeError "mixed types in list literal"
        | listLiteral _ _ = let exception ThisCan'tHappen in raise ThisCan'tHappen end
(*#line 25 "tuschemeans.nw"*)      fun ty(LITERAL n) = literal n
        | ty(VAR v) = find(v, Gamma)
        | ty(SET (x, e)) =
...
(*#line 38 "tuschemeans.nw"*)        | ty (WHILEX (e1, e2)) =
            let val tau1 = ty e1
                val tau2 = ty e2
...
(*#line 48 "tuschemeans.nw"*)        | ty(APPLY (f, actuals)) =
            ...
            in  case ty f
                  of FUNTY (formals, result) =>
                        if eqTypes(atys, formals) then
                            ...
                            raise TypeError ("function of type " ^ typeString (ty f) ^
                                             " received arguments of types (" ^
                                             spaceSep (map typeString atys) ^ ")")
                   | FORALL _ => raise TypeError ("function is polymorphic; " ^
                                                  "instantiate before applying")
                   | _ => raise TypeError ("tried to apply non-function of type " ^
                                           typeString (ty f))
            end
(*#line 65 "tuschemeans.nw"*)        | ty(LETX (LETSTAR, [], body)) = ty body
        | ty(LETX (LETSTAR, (b :: bs), body)) = ty (LETX (LET, [b], LETX (LETSTAR, bs, body)))
(*#line 68 "tuschemeans.nw"*)        | ty(IFX (e1, e2, e3)) =
            if eqType(ty e1, booltype) then
                let val (t2, t3) = (ty e2, ty e3)
                in  if eqType(t2, t3) then t2
                    else raise TypeError ("incompatible types " ^ typeString t2 ^
                                          " and " ^ typeString t3 ^
                                          " across branches of if")
                end
            else
                raise TypeError ("condition in if has type " ^ typeString (ty e1))
(*#line 79 "tuschemeans.nw"*)        | ty (BEGIN es) =
             let val bodytypes = map ty es
                 fun last tau []     = tau
                   | last tau (h::t) = last h t
             in  last unittype bodytypes
             end
(*#line 86 "tuschemeans.nw"*)        | ty(LAMBDA (formals, body)) =
            let val newGamma =
                  List.foldl
                  (fn ((n, ty), Gamma) => bind(n, asType (ty, Delta), Gamma))
                  Gamma formals
                val bodytype = typeof(body, Delta, newGamma)
            in  FUNTY (map (fn (n, ty) => ty) formals, bodytype)
            end
(*#line 95 "tuschemeans.nw"*)        | ty(LETX (LET, bs, body)) =
            let val Gamma' = List.foldl
                             (fn ((x, e), newGamma) => bind(x, ty e, newGamma))
                             Gamma bs
            in  typeof(body, Delta, Gamma')
            end
(*#line 102 "tuschemeans.nw"*)        | ty(TYAPPLY (e, args)) = instantiate (ty e, args, Delta)
        | ty(TYLAMBDA (alphas, e)) =
            let val Delta' =
                  foldl (fn (x, Delta') => bind(""^x, TYPE, Delta')) Delta alphas
            in  FORALL (alphas, typeof(e, Delta', Gamma))
            end
  in  ty e
  end
(*#line 114 "tuschemeans.nw"*)fun elabdef (d, Delta, Gamma) =
  case d
    of VAL (name, e) =>
     ...
     | DEFINE (name, tau, lambda as (formals, body)) =>
         ...
         in  elabdef (VALREC (name, funty, LAMBDA lambda), Delta, Gamma)
         end
     | VALREC (name, tau, e) =>
         let val tau    = asType (tau, Delta)
             val Gamma' = bind(name, tau, Gamma)
             val tau'   = typeof(e, Delta, Gamma')
         in  if not (eqType (tau, tau')) then
               raise TypeError ("identifier " ^ name ^ " is declared to have type " ^
                                typeString tau ^ " but has actual type " ^
                                typeString tau')
             else if appearsUnprotectedIn (name, e) then
               raise TypeError ("identifier " ^ name ^
                                " is evaluated in right-hand side of val-rec")
             else
     ...
     (* definition of [[complaintOfExn]], why we don't like an exception 365b *)
            fun complaintOfExn exn =
              let fun template exn =
                    case exn
                      of IO_Io { name, ...} => "I/O error <at loc>: " ^ name
